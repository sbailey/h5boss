...
size_t plugmap_sizes[NFIELDS_PLUGMAP] = {
  sizeof(H5T_NATIVE_INT32)*5,
  sizeof(H5T_C_S1)*6,
  sizeof(H5T_NATIVE_DOUBLE),
  sizeof(H5T_NATIVE_DOUBLE),
  sizeof(H5T_NATIVE_FLOAT)*5,
  sizeof(H5T_NATIVE_FLOAT),
  sizeof(H5T_NATIVE_FLOAT),
  sizeof(H5T_NATIVE_FLOAT),
  sizeof(H5T_C_S1)*16,
...
size_t zbest_sizes[NFIELDS_ZBEST] = {
  ...
  sizeof(H5T_NATIVE_FLOAT),
  sizeof(H5T_C_S1)*24,
  sizeof(H5T_NATIVE_INT32)*10,
  sizeof(H5T_NATIVE_INT32),
  sizeof(H5T_NATIVE_FLOAT)*10,
  sizeof(H5T_NATIVE_FLOAT)*100,
  sizeof(H5T_NATIVE_FLOAT),
  sizeof(H5T_NATIVE_FLOAT),
  sizeof(H5T_NATIVE_FLOAT),
  sizeof(H5T_NATIVE_FLOAT),
...
size_t matchflux_sizes[NFIELDS_MATCHFLUX]={
  ...
  sizeof(H5T_NATIVE_FLOAT)*5,
  sizeof(H5T_NATIVE_FLOAT)*5,
  sizeof(H5T_NATIVE_FLOAT)*40,
  sizeof(H5T_NATIVE_FLOAT)*40,
  sizeof(H5T_NATIVE_INT32)*5,
  sizeof(H5T_NATIVE_INT32)*5,
  sizeof(H5T_NATIVE_FLOAT)*5,
  sizeof(H5T_NATIVE_FLOAT)*5,
  sizeof(H5T_NATIVE_DOUBLE)*5,
  sizeof(H5T_NATIVE_INT32),
  sizeof(H5T_NATIVE_INT32),
  sizeof(H5T_NATIVE_INT32),
  sizeof(H5T_NATIVE_INT32),
...
void compound_read_catalog(struct Catalog * cl, hid_t dst_file, int it, int write,int rank){
  ...
  //split keys into group/dataset_table
  //open the group,
  //plate/mjd =cl->plate_mjd[it], now construct the groups, which should be:
  /*plate/mjd  for plugmap
   *plate/mjd  for zbest
   *plate/mjd  for zline
   *plate/mjd/photo for match
   *plate/mjd/photo for matchflux
   *plate/mjd/photo for matchpos
  */
  char photo_gp[20];
  strncpy(photo_gp,cl->plate_mjd[it],sizeof(cl->plate_mjd[it])-1);
  photo_gp[sizeof(cl->plate_mjd[it]-1)]='\0';
  strcat(photo_gp,photo);
  //for i=0-2 in catalog_array, open group at plate/mjd, then open table
  //for i=3-5 in catalog_array, open group at plate/mjd/photo, then open table
  int icat;
  void * data_catalog;
  for (icat=0;icat<n_catalog;icat++){
    ...
    data_catalog = malloc(record_size);
    if(icat<3){//read, write groups at plate/mjd, open tables: plugmpa, zbest, zline
    ...
    else {//read,write groups at plate/mjd/photo, open tables: match, matchflux,matchpos
...
//void compound_write(char * dst_file,char * grp, const char * table_name, hsize_t nrecords,size_t type_size, const size_t * field_offsets, const size_t * field_sizes,void * data){
void compound_write_catalog(hid_t dst_file,char * grp, const char * table_name, hsize_t nrecords,size_t type_size, const size_t * field_offsets, const size_t * field_sizes,void * data){
  herr_t ierr=-1;
  hid_t igroup=-1;
  hid_t ifile=-1;
  //ifile= H5Fopen(dst_file,H5F_ACC_RDWR,H5P_DEFAULT);
